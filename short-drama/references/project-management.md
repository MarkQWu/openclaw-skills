...
`/开始` 首步必须判断当前工作目录状态。想直接新建另一本剧本用独立命令 `/新建 <项目名>`（无论 cwd 状态如何均适用）。

| cwd 状态 | 行为 |
|---|---|
| 有 `.drama-state.json`（`currentStep` 非空，真实旧项目）| 自动加载旧项目，输出"欢迎回来：《{dramaTitle}》，当前阶段 {currentStep}，已完成 {completedCount}/{totalEpisodes}。继续创作：`/分集 {下一集}` 或对应阶段命令。如要新建剧本，输入 `/新建 <新项目名>`"。停在此等用户指令，不进 Step 2（零迁移） |
| 有 `.drama-state.json`（`currentStep` 为空，`/新建` 预写的 stub） | 输出"欢迎来到《{dramaTitle}》，我们来定方向。"直接进 Step 2 锁定观众，不再问项目名 |
...
`.drama-state.json` 完整字段定义（17 字段，schema 稳定，本次 v1.12 未新增字段）：
...
**向后兼容约束**：新字段可追加（读取到 null 视为未设置），已有字段不改名/不改类型。brainstorm 内容和配置决策历史写入 `brainstorm.md` 文件而非 state，降低 schema 变更压力。

## 迭代记录

- 2026-04-20 首版：新用户 WorkBuddy Windows 提出项目管理诉求，引入项目目录约定 + 向后兼容 + `/项目列表` `/项目状态` 两条命令。
- 2026-04-20 v1.12.1 patch：`/开始 --new <项目名>` flag 被独立命令 `/新建 <项目名>` 取代。原因：小白用户反馈 flag 语法不直观。`/开始` 职责纯化为"继续/从头建"，新建另一本用 `/新建`。
