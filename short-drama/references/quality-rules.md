...
以下三种常见套路是**通用叙事硬伤**（非题材限定），自检时按 AI Slop 维度或连贯性维度扣分：

| 坏模式 | 问题 | 处理方式 |
|--------|------|---------|
| **无动机背叛** | 闺蜜/兄弟背叛时没有合理利益驱动，变成"单纯坏"，读者不买账 | 任何背叛必须铺垫具体利益驱动或价值观冲突——嫉妒/利益分配/身份焦虑/家族压力，选一条明写 |
| **扇耳光解决冲突** | 暴力升级过快直接动手，同时触及平台暴力尺度合规 | 改为语言打脸或资源碾压（搞垮对方生意、断其财路、公开揭穿）。物理冲突只能在角色弧线的"无可忍让"关键点出现一次 |
| **苦情戏哭诉拖节奏** | 角色长时间哭诉/独白/回忆苦难，情节停滞，观众跳过 | 压缩苦情篇幅到 1 场以内，快速进入反击/决策/行动。苦难通过 △ 段落的"手攥紧衣角""指关节发白"等外化动作交代，而非长独白 |

**自检时**：发现此类坏模式 → 按"连贯性"维度扣分（无动机背叛是连贯性硬伤），或按"AI Slop"维度扣分（扇耳光和苦情戏拖节奏是节奏硬伤）。
...
| 格式 | 场景头完整性、景别标注、音乐提示、特殊标记、**模板合规**（不含模板外区块如导演手记/创作心得） |
