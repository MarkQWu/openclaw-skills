...
- **`CHECK_FAILED 7d`** → 网络连续 7 天无法检查更新，显示淡提示：
  > ℹ️ 已超过 7 天未能检查更新（网络问题）。可手动运行 `/更新` 检查，或确认网络连通后自动恢复。
...
4. **锚定声明**：每个命令的输出严格使用对应语言版本的模板，不混用
...
- **术语规则**：
  - 中文模式：使用中文术语（全景/中景/特写/内景/外景），行业通用缩写可保留（BGM、CTA）
  - 英文模式：使用英文术语（WIDE SHOT/MEDIUM SHOT/CLOSE-UP/INT./EXT.）
- **Emoji 规则**：标记型 emoji（🔥💰⚡🎣📺♪）保留（两种语言通用），装饰型 emoji 不添加
...
> **总集数**：{N}集 | 🔥 关键剧情：{X}集（{百分比}%） | 💰 付费卡点：{Y}集（{百分比}%）

---

## 第一幕：建置（第{X}-{Y}集）

第1集：{集标题} —— {核心冲突或爽点一句话描述} 🔥
第2集：{集标题} —— {核心冲突或爽点一句话描述}
第3集：{集标题} —— {核心冲突或爽点一句话描述} 💰
...
...
5. 提示用户：
> ✅ 升级完成！新版本在**下次对话**中生效（当前对话仍使用旧版 SKILL.md）。
