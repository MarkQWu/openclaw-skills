...
**用途：** 爆款解构 → 题材迁移 → 热点融入 → 填表仿写。从参考剧本提取结构 DNA，迁移到新题材，融入当代热点，用填表法逐集生成正文。

**与 `/重构` 的区别：** `/重构` 只产出创意骨架（理念层，不建文件）；`/仿写` 产出完整剧集正文（执行层），带热点研究 + 填表硬约束 + 防结构漂移，文件输出与 `/分集` 一致。

**前置条件：** 无特殊要求。无活跃项目时自动触发新建（见 Phase 0）。

**加载参考：** `references/imitation-protocol.md`（执行任何步骤前先 Read 该文件）

---

**Phase 0：项目准备（每次 /仿写 必先执行）**

1. **检查活跃项目**：
   - 有活跃项目 → 输出"当前项目：《X》，将在此项目下创建仿写产出。继续？（回复"继续"或给我参考剧本）" → 用户确认后进 Phase 1
   - 无活跃项目 → 直接询问："请给这个仿写项目起个名字（新建）："→ 按 `/新建` 流程创建项目（建目录 + 写 stub state，见 `references/project-management.md#stub-state`）→ 自动进 Phase 1
   - **不展示现有项目选择列表**（用户要仿写就是新开一个本，不要求匹配已有项目）

2. **参考剧本来源**：用户可以在同一条消息里一起提供参考剧本（粘贴文本/文件路径/剧名），也可以在 Phase 0 确认后再提供。
...
2. **提取六维 DNA**（详细维度定义见 `references/imitation-protocol.md#Phase-1`）：
   ...
   - 维度 2：压迫机制（权力来源 + 压迫边界 + 压迫者的盲区）
   - 维度 3：情感引爆点分布（类型 + 集数位置 + 全剧占比）
   - 维度 4：爽点密度 & 类型（每 5 集密度 + 主力爽点类型）
...
1. **变体方向生成**：基于 source-skeleton.md 生成 3 个变体方向（保留结构逻辑，替换题材赛道 + 场景 + 人物弧线具体化身）。每个变体一行描述：`变体 X：[赛道] — 压迫机制迁移为[A]，身份密码变体为[B]，核心张力变为[C]`

2. **用户选择变体**（用户回复编号或提出调整）

3. **时代底色推导**（LLM 推导，不是让用户回答）：
   - 读 source-skeleton.md + 已选变体 → 推导 2-3 个候选社会母体
   - 推导逻辑：压迫机制权力来源→集体焦虑映射；身份密码形式→社会中哪类人需要隐藏；清算手段→观众正义投射
   - 输出格式："基于源剧骨架（[摘要]）+[变体方向]，时代底色候选：A) [母体] — [为什么骨架指向此] B) … 请确认一个，或告诉我你想要的方向"
   - 用户确认后记录到 new-setting.md `时代底色` 字段

3.5. **目标平台推导**（LLM 推导，用户确认）：基于变体方向 + 时代底色推断抖音/快手/红果/出海，输出"平台名 — 一句话节奏/集长/付费墙约束"，用户确认后写入 new-setting.md `目标平台` 字段（详见 imitation-protocol.md Step 2.3.5）

4. **热点研究**：基于确认的时代底色，并行 3 类 WebSearch（社会话题 / 情感共鸣 / 网络热梗），每类精选 3-5 条 + 一句话说明相关性。让用户选 2-3 条。

5. **融入层次原则**（执行时强制）：优先世界观层（热点塑造故事世界规则）> 人物动机层（热点驱动角色选择）> 剧情事件层（热点作为具体事件）。**禁止**热点作为台词名词标签出现但与剧情逻辑无关。

5.5. **综合推导（无需用户输入）**：用户确认热点后，LLM 一次性自动推导并输出确认清单（详见 imitation-protocol.md Step 2.4.5）：
   - **核心人物焕新**：4 类角色（主角/反派/情感对象/恶毒配角）× 新身份职业/性格关键词/独特动作口头禅
   - **关键场景/道具映射**：原剧高情绪载体 → 新剧等价场面/道具（6-10 行）
   - **剧名候选 + Logline**：2 个剧名候选 + 一句话 Logline（主角身份+触发事件+核心关系+情绪基调）
   - **预判切片爆点**：3-5 个高光时刻 + 大致集数 + 切片类型（打脸/揭秘/反转/告白/清算）
   - 目标平台（Step 2.3.5 已确认）

6. **输出文件**：写 `imitation/new-setting.md`（剧名/Logline / 题材赛道 / 时代底色 / 目标平台 / 融入热点表[含靶向情绪] / 核心人物焕新表 / 骨架迁移映射表 / 关键场景道具映射 / 预判切片爆点）

---

**Phase 3：逐集填表仿写**

用户可指定集数（`/仿写 5` = 第 5 集，`/仿写 5-8` = 第 5-8 集），默认生成 1 集。

1. **前置加载**：Read `imitation/source-skeleton.md` + `imitation/new-setting.md`。**不重读源剧正文**（防内容污染，强制从结构逻辑生成）
...
2. **生成对齐表**（每集必做，先表后文）：
   ...
   | 冲突核心 | … | … |
   | 爽点类型 | 有/无 + 类型 | … |
   | 热点融入 | N/A | 本集融入[热点X]，以[融入层次]方式 |
   | 付费卡点 | 有/无 + 诱饵情绪 | … |
   | 台词风格锚 | … | 对照 new-setting.md 角色焕新表的独特动作/口头禅 |
   | 人物动作锚 | [源剧对应角色标志动作] | [本集哪个角色（来自 new-setting.md 角色焕新表）/哪个场次触发独特动作/口头禅] |
   | 切片承载 | N/A | 本集是否承载预判切片X，注明场次；否则标「铺垫/过渡集」 |
   ```
