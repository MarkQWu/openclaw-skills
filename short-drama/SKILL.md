...
**首次使用确认**：第一次执行 `/分镜` 时（`.drama-state.json` 中无 `shotDensity` 字段），读取输入后向用户确认：

```
📐 镜头节奏确认：

默认按「节奏对比」自动拆解（高潮快切 30-45 镜/分钟，铺垫慢拍 12-20 镜/分钟）。
你也可以：
  - 指定每分钟镜头数（如「每分钟 25 个」）
  - 指定总镜头数（如「这集 50 个镜头」）
  - 直接回车 = 自动节奏
...
用户选择后记录到 `.drama-state.json` 的 `shotDensity` 字段，同一项目后续不再询问。
...
5. **生成双 Prompt**（每个镜头生成两条，参考 storyboard-guide.md）：
