...
2. **称呼关系表**（N×N 矩阵，参考 realism-checklist.md）：
   - 行 = 说话者，列 = 被称呼者
...
4. **角色弧线设计**：每个主要角色从第一集到最后一集的变化轨迹

5. **感情线弧线**：男女主关系发展的关键节点（集数标注）
...
7. **反派体系**（按 villain-design.md 的4层结构）：
   - 小反派（前期炮灰）
   - 中反派（中期主要对手）
   - 大反派（终极 Boss）
   - 隐藏反派（反转用）
...
| 台词 | 有无废话、角色区分度、是否口语化自然、**称呼是否匹配层级关系、场景台词是否符合场景逻辑**（参考 realism-checklist.md） |
