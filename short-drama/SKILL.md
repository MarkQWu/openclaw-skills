...
2. **确定输出语言**：
   - `language: "zh-CN"` 或未设置 → 使用中文模板
   - `language: "en"` → 使用英文模板
3. **确定输出格式**：
   - `mode: "domestic"` 或未设置 → 国内模式格式
   - `mode: "overseas"` → 出海模式格式（好莱坞行业标准）
4. **锚定声明**：重读本命令的输出模板区块，严格按模板输出，不混用语言版本

### 格式封闭原则（强制）

- **禁止添加模板外内容**：每个命令的输出必须严格遵循其模板定义，不得自行添加「导演手记」「创作心得」「补充说明」「拍摄建议」等模板中未定义的区块
  - 反面示例：在分集剧本末尾添加「导演手记：本集重点表现...」或「创作思路：这一集的设计意图是...」
  - 正确做法：分集剧本以 [预告] 和集末自查结束，不追加其他区块
- **禁止格式漂移**：不得将某个命令的格式特征（如表格）泛化到其他命令。每个命令有独立的输出模板，严格按各自模板输出
- **禁止中英混杂**：中文模式下不出现英文行业术语（如 INT./EXT.），英文模式下不出现中文
- **术语规则**：
  - 中文模式：使用中文术语（远景/全景/中景/近景/特写/大特写/内景/外景），行业通用缩写可保留（BGM、CTA）
  - 英文模式：使用英文术语（WIDE SHOT/MEDIUM SHOT/CLOSE-UP/INT./EXT.）
...
- **秒级节奏锚点**：每集必须包含以下三个节奏锚点，在剧本中用 `[锚点]` 标记：
...
- 49-62：良好，建议微调
- 30-48：及格，需要修改后重新自检
- **<30（即 29 分及以下）：不合格，建议重写。[注意] 自检总分 <30 时，`/导出` 将被阻断，必须修改后重新自检达标**
