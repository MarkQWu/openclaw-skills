...
**检查维度（7项，每项 1-10 分）：** 节奏、爽点、台词、格式、连贯性、反抽象、AI Slop。细则见 `references/quality-rules.md#自检维度细则`
...
**结束提示：** 根据评分给出建议（重写/微调/通过）。总分 <30 时额外警告：`[注意] 本集自检不合格（{X}/70），/导出 将被阻断。请修改后重新 /自检`
