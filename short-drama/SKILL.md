...
默认密度：**20-30 镜/分钟**（短剧标准节奏，ASL 2-3 秒）。读取输入后、拆解前，向用户确认：

```
📐 镜头密度选择（每分钟镜头数）：

  A. 短剧标准（默认）— 20-30 镜/分钟（ASL 2-3s）
     适合：爽剧、霸总、逆袭、悬疑，大部分竖屏短剧
  B. 高密度快切 — 30-45 镜/分钟（ASL 1-2s）
     适合：动作追逐、蒙太奇段、开头 hook、打脸名场面
  C. 情感舒缓 — 12-20 镜/分钟（ASL 3-5s）
     适合：情感、家庭、慢热剧情、氛围渲染
  D. 自定义 — 输入具体数字（如 "25"）
...
**节奏对比原则**：不匀速快切。冲突/反转段自动提速（向 B 靠拢），舒缓/过渡段自动减速（向 C 靠拢），形成快-慢-快波浪节奏。

用户选择后记录到 `.drama-state.json` 的 `shotDensity` 字段，后续同一项目不再重复询问。

密度计算方式：目标总镜头数 = 预估总时长（秒）÷ 60 × 选定密度。拆解时以此为参考，允许 ±20% 浮动。

**行业参考（供对比）：**
- 传统电视剧：10-15 镜/分钟（ASL 4-6s）
- 当代动作大片（漫威/变形金刚）：15-20 镜/分钟（ASL 3-4s）
- 微短剧（竖屏爽剧）：20-30 镜/分钟（ASL 2-3s）← 默认
- TikTok 广告/极速剪辑：30-60 镜/分钟（ASL 1-2s）
...
3. **拆解为镜头序列**：
   - 逐场次分析
   - 根据选定的镜头密度拆解，每个场次的镜头数 = 场次预估时长（秒）÷ 60 × 密度
...
4. **生成双 Prompt**（每个镜头生成两条，参考 storyboard-guide.md）：
   - **文生图 Prompt**（30-60 词）：描述冻结瞬间——`{角色外观前缀} + {姿态定格} + {场景环境} + {光影/色调} + {风格} + {质量约束}`
   - **图生视频 Prompt**（15-40 词）：描述动态变化——`{镜头运动} + {主体动作} + {环境变化}`
   - One-Move Rule：每个镜头只描述一个核心动作
   - 情绪描述 → 自动转换为物理动作（参考 storyboard-guide.md 对照表）
   - 如有角色卡 → 文生图 prompt 自动拼接角色前缀
   - 质量约束后缀自动追加：`4K, 画面清晰, 电影质感`；含角色时追加 `保持面部特征一致, 正常人体结构`
5. **输出分镜表 + Prompt 汇总**
...
> 文生图 prompt → 即梦/MidJourney 生成关键帧静态图
> 图生视频 prompt → 即梦/Seedance 驱动静态图变视频

### 镜头 1
**文生图：** {冻结画面描述，含角色前缀 + 场景 + 光影 + 质量约束}
**图生视频：** {镜头运动 + 主体动作 + 环境变化}
...
**Prompt 质量自检（生成后自动执行）：**
- [ ] 文生图 prompt 是否描述的是一个冻结瞬间（无动作链）？
- [ ] 图生视频 prompt 是否只描述动态变化（无重复外观）？
- [ ] 文生图 30-60 词？图生视频 15-40 词？
- [ ] 每条 prompt 是否只有一个核心动作？
- [ ] 包含角色的镜头是否拼接了角色外观描述 + 稳定性约束？
- [ ] 是否有抽象描述未转为物理动作？
- [ ] 是否有质量约束后缀（4K, 画面清晰, 电影质感）？
- [ ] 描述内容在该景别/角度下是否物理上可见？
