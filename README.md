...
**v1.5.0 起支持自动更新检测**：每次开新对话时，skill 会自动检查是否有新版本。有更新时会询问你：立即升级、开启自动更新、暂不（24h 后再提醒，逐次递增到 7 天）、或永远不提醒。

**方式一：有更新时直接选「立即升级」**（推荐）

新版本发布后，你下次使用 skill 时会自动弹出提示，选「立即升级」即可。也可以随时手动输入 `/更新` 主动检查。

> 💡 首次升级到 v1.5.0 的老用户需要先用下面的方式二或三手动更新一次，之后就自动了。
...
**Q：每次开对话都提醒更新，太烦了**
A：选「暂不」会自动递增退避（24h→48h→7d），选「永远不提醒」彻底关闭。之后想更新随时输入 `/更新`。

**Q：以前用「让 AI 帮你更新」的方式还能用吗？**
A：能用，但推荐升级到 v1.5.0 后用 `/更新` 命令，更简单也更可靠。
